#ifndef SPI_O_H
#define SPI_O_H

#include"DSP2833x_Project.h"
#include <stdio.h>

#define  SPI_FLASH_ID              0xEF4017  //W25Q64

/*******************************************************************************
 * w25q64_gpio_init
 * 描述  ：模拟初始化spi_w25q64
 * 输入  ：void
 * 输出  ：void
 * 调用  ：内部调用
 * 备注  ：
 *******************************************************************************/
void w25q64_gpio_init();



// /*******************************************************************************
//  * 函数名：SPI_CS_HIGH
//  * 描述  ：CS输出高电平
//  * 输入  ：void
//  * 输出  ：void
//  * 调用  ：内部调用
//  * 备注  ：
//  *******************************************************************************/
// void SPI_CS_HIGH(void);


// /*******************************************************************************
//  * 函数名：SPI_CS_LOW
//  * 描述  ：CS输出低电平
//  * 输入  ：void
//  * 输出  ：void
//  * 调用  ：内部调用
//  * 备注  ：
//  *******************************************************************************/
// void SPI_CS_LOW(void);


// /*******************************************************************************
//  * 函数名：SPI_SLK_HIGH
//  * 描述  ：SLK输出高电平
//  * 输入  ：void
//  * 输出  ：void
//  * 调用  ：内部调用
//  * 备注  ：
//  *******************************************************************************/
// void SPI_SLK_HIGH(void);


// /*******************************************************************************
//  * 函数名：SPI_SLK_LOW
//  * 描述  ：SLK输出低电平
//  * 输入  ：void
//  * 输出  ：void
//  * 调用  ：内部调用
//  * 备注  ：
//  *******************************************************************************/
// void SPI_SLK_LOW(void);


// /*******************************************************************************
//  * 函数名：SPI_SLK_HIGH
//  * 描述  ：SI输出高电平
//  * 输入  ：void
//  * 输出  ：void
//  * 调用  ：内部调用
//  * 备注  ：
//  *******************************************************************************/
// void SPI_DI_HIGH(void);


// /*******************************************************************************
//  * 函数名：SPI_DI_LOW
//  * 描述  ：SI输出低电平
//  * 输入  ：void
//  * 输出  ：void
//  * 调用  ：内部调用
//  * 备注  ：
//  *******************************************************************************/
// void SPI_DI_LOW(void);


// /*******************************************************************************
//  * 函数名：Read_DO_Level
//  * 描述  ：读取DO电平
//  * 输入  ：void
//  * 输出  ：void
//  * 调用  ：内部调用
//  * 备注  ：
//  *******************************************************************************/
// Uint16 Read_DO_Level(void);


// /*******************************************************************************
//  * 函数名：SPI_SendByte
//  * 描述  ：SPI读取数据
//  * 输入  ：data
//  * 输出  ：Outdata
//  * 调用  ：内部调用
//  * 备注  ：通过发送两个个字节的数据并同时接收两个字节的数据，实现了数据的读取操作。
//  *******************************************************************************/
// Uint16 SPI_Send_2Byte(Uint16 data);

// /*******************************************************************************
//  * 函数名：W25Q64_ChipErase
//  * 描述  ：芯片擦除函数
//  * 输入  ：void
//  * 输出  ：void
//  * 调用  ：内部调用
//  * 备注  ：在执行写入操作前要进行擦除
//  *******************************************************************************/
// void W25Q64_ChipErase(void);

// /*******************************************************************************
//  * 函数名：W25Q64_ReadID
//  * 描述  ：W25Q64读取设备ID
//  * 输入  ：*ID	存储ID的变量	
//  * 输出  ：void
//  * 调用  ：内部调用
//  * 备注  ：EF4017
//  *******************************************************************************/
// void W25Q64_ReadID(Uint32 *ID);

// /*******************************************************************************
//  * 函数名：W25Q64_ReadData
//  * 描述  ：W25Q64读数据
//  * 输入  ：void
//  * 输出  ：void
//  * 调用  ：
//  * 备注  ：
//  *******************************************************************************/
// void W25Q64_ReadData(Uint32 address, Uint16 *data, Uint16 dataSize);

// /*******************************************************************************
//  * 函数名：W25Q64_PageProgram
//  * 描述  ：W25Q64页写操作
//  * 输入  ：address  要写入的起始地址
//            *data     要写入的数据缓冲区指针。
//            dataSize  要写入的数据大小，单位为字节。
//  * 输出  ：void
//  * 调用  ：内部调用
//  * 备注  ：
//  *******************************************************************************/
void W25Q64_PageWrite(Uint32 address, Uint16 *data, Uint16 dataSize);

void SPI_Init();
void SPI_SET_MOSI(Uint16 val);
unsigned int SPI_ReadWriteByte(unsigned int data);
Uint32 FLASH_ReadJEDE_ID();
Uint32 FLASH_ReadDeviceID();
void SPI_FLASH_WriteEnable();
void SPI_FLASH_WaitForWriteEnd();
void FLASH_PageWrite(Uint32* pBuffer, Uint32 WriteAddr, Uint16 NumByteToWrite);
void SPI_FLASH_BufferWrite(Uint32* pBuffer, Uint32 WriteAddr, Uint16 NumByteToWrite);
void SPI_FLASH_BufferRead(Uint32* pBuffer, Uint32 ReadAddr, Uint16 NumByteToRead);
//void FLASH_PageWrite(long double* pBuffer, Uint32 WriteAddr, Uint16 NumByteToWrite);
//void SPI_FLASH_BufferWrite(long double* pBuffer, Uint32 WriteAddr, Uint16 NumByteToWrite);
//void SPI_FLASH_BufferRead(long double* pBuffer, Uint32 ReadAddr, Uint16 NumByteToRead);
void SPI_FLASH_SectorErase(Uint32 SectorAddr);
void FLASH_32KErase(Uint32 addr);
void FLASH_64KErase(Uint32 addr);
void SPI_FLASH_ChipErase();
void SPI_Flash_WAKEUP();

#endif
